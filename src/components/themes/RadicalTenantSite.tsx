...
export function ServiceDetailSite({b,service,gallery,media,services,hours}:{b:any;service:any;gallery:any[];media:any[];services:any[];hours:any[]}){
  ...
  const wrap=(inner:React.ReactNode)=><div className={`radical profession-${b.business_type} mode-${mode} scheme-${effectiveScheme} font-${b.font_family||'serif'}`} style={{'--accent':accent,'--brand':accent,'--bg':schemeColors.bg,'--text':schemeColors.text}as React.CSSProperties}>{inner}<WhatsApp b={b}/><GoogleReviewLink b={b}/></div>;
...
/* ================= NOVA — kullanıcının paylaştığı "3D Orbit Gallery" (React
   Three Fiber) referansının BİREBİR aynısını hero'da kullanan tema: toz
   parçacıklarından bir küre, etrafında dönen fotoğraf halkası, sürükle-çevir/
   yakınlaştır kamera kontrolü. Sahnenin kendisi ayrı bir dosyada (NovaScene.tsx,
   ssr:false dinamik import) — bkz. oradaki yorum için performans/güvenlik
   notları (InstancedMesh, pan kapalı). */
function Nova(p:P){
  const{b}=p;
  const hourRows=groupedHourRows(p.hours||[]);
  ...
  return <main id="top" className="tNova">
    ...
    <section className="nvHero">
      ...
      <div className="nvHeroInner">
        ...
        <div className="nvHeroActions">
          <a className="nvBtnSolid" href="#randevu">{b.booking_button_text||'Randevu Al'} →</a>
    ...
    <section id="hizmetler" className="nvServices">
      ...
      <div className="nvServiceGrid">
        {p.services.map((s,i)=><Reveal as="article" i={i} key={s.id} className="nvServiceCard">
          <div className="nvServiceHead"><h3>{s.name}</h3>{b.show_prices&&s.price!=null&&<b>₺{Number(s.price).toLocaleString('tr-TR')}</b>}</div>
          {s.description&&<p>{s.description}</p>}
          <small>🕐 {s.duration_minutes} dk</small>
          {hasServiceDetail(s)&&<a className="nvServiceDetailLink" href={`/site/${b.slug}/hizmet/${s.slug}`}>Detaylı İncele →</a>}
        </Reveal>)}
      </div>
    </section>
    ...
    <footer className="nvFooter">
      <div className="nvFooterGrid">
        <div><a className="nvBrand" href="#top">{b.logo_url?<img src={b.logo_url} alt={b.name}/>:<i>{b.name?.[0]}</i>}<b>{b.name}</b></a><p>{dec(b,'nv_footerTagline','Zamanın en değerli hâli: kendine ayırdığın an.')}</p></div>
        <div><small>ÇALIŞMA SAATLERİ</small>{hourRows.map((r,i)=><div key={i} className="nvHoursRow"><span>{r.label}</span><span>{r.value}</span></div>)}</div>
  ...
  return <main id="top" className="tDeneyim" ref={scRef as any}>
    ...
    <section className="dxScene dxHero" data-dx-ch="0" style={{order:10}}>
      ...
      <div className="dxSceneInner dxHeroInner">
        ...
        <h1 className="dxSplit">{dxWords(b.hero_title?(b.hero_highlight?`${b.hero_title} ${b.hero_highlight}`:b.hero_title):'Randevunu al, deneyime başla')}</h1>
        {b.hero_description&&<p>{b.hero_description}</p>}
...
function Split(p:P){
  const{b}=p;
  const hourRows=groupedHourRows(p.hours||[]);
  const shots=[...p.gallery.map((g:any)=>g.image_url),...p.media.filter((m:any)=>m.type!=='video').map((m:any)=>m.url)].filter(Boolean) as string[];
  const heroImg=b.cover_url||shots[0]||'';
  const aboutImg=shots[1]||shots[0]||'';
  let wa=String(b.whatsapp_phone||b.phone||'').replace(/\D/g,'');if(wa.startsWith('0'))wa='90'+wa.slice(1);
  const[reviews,setReviews]=useState<any[]>([]);
  useEffect(()=>{fetch(`/api/ratings/${b.id}`).then(r=>r.json()).then(d=>{if(d?.enabled&&Array.isArray(d.reviews)&&d.reviews.length)setReviews(d.reviews.slice(0,8))}).catch(()=>{})},[b.id]);
  const avgScore=reviews.length?(reviews.reduce((s,r)=>s+(r.stars||5),0)/reviews.length).toFixed(1):'5.0';
  const[ready,setReady]=useState(false);
  const rightColRef=useRef<HTMLDivElement>(null);
  const aboutTextRef=useRef<HTMLDivElement>(null);
  useEffect(()=>{const id=requestAnimationFrame(()=>setReady(true));return()=>cancelAnimationFrame(id)},[]);
  useEffect(()=>{
    const target=aboutTextRef.current,panel=rightColRef.current;
    if(!target||!panel)return;
    const io=new IntersectionObserver(es=>es.forEach(e=>panel.classList.toggle('spShowAbout',e.isIntersecting)),{threshold:.5});
    io.observe(target);
    return()=>io.disconnect();
  },[]);
  return <main id="top" className="tSplit">
    <nav className="spNav">
      <a className="spBrand" href="#top">{b.logo_url?<img src={b.logo_url} alt={b.name}/>:<i>{b.name?.[0]}</i>}<b>{b.name}</b></a>
    ...
    <section className="spSplit spHeroAbout">
      <div className="spLeftCol">
        ...
        <div className="spStickySpacer" aria-hidden="true"/>
      </div>
      <div className={`spRightCol${ready?' in':''}`} ref={rightColRef} aria-hidden="true">
        {heroImg&&<div className="spVisualLayer spHeroLayer" style={{backgroundImage:`url(${heroImg})`}}/>}
        {aboutImg&&<div className="spVisualLayer spAboutLayer" style={{backgroundImage:`url(${aboutImg})`}}/>}
      </div>
    </section>
    ...
    {p.services.length>0&&<section id="hizmetler" className="spServices">
      ...
      <div className="spBento">
        {p.services.map((s,i)=><Reveal as="article" className={`spBentoCard${i===0?' spBentoBig':''}`} i={i} key={s.id}>
          <span className="spIndex">{String(i+1).padStart(2,'0')}</span>
          <h3>{s.name}</h3>
          {s.description&&<p>{s.description}</p>}
          <footer>
            <span>{s.duration_minutes} dk</span>
    ...
    <section id="yorumlar" className="spReviews">
      ...
      <div className="spReviewsFlow">
        {reviews.length?reviews.map((r,i)=><Reveal as="article" className="spReviewCard" i={i} key={i}>
          <p>&ldquo;{r.comment}&rdquo;</p>
          <footer><b>{r.customer_name||'Müşterimiz'}</b>{r.service_label&&<span>{r.service_label}</span>}</footer>
        </Reveal>):<p className="spReviewsEmpty">İlk yorumlar geldiğinde burada, bu şeritte akacak.</p>}
      </div>
    </section>
    ...
    <section id="randevu" className="spSplit spBooking">
      ...
      <div className="spRightCol spBookingInfo">
        <div>
          <span className="spKick">ÇALIŞMA SAATLERİ</span>
          {hourRows.map((r,i)=><div className="spHourRow" key={i}><span>{r.label}</span><span>{r.value}</span></div>)}
        </div>
        {b.address&&<div className="spLocation">
          <span className="spKick">KONUM</span>
          <p>{b.address}</p>
          {b.show_map!==false&&<iframe className="spMap" src={`https://www.google.com/maps?q=${encodeURIComponent(b.address)}&output=embed`} loading="lazy" title="Konum"/>}
        </div>}
...
/* Panelde "Google değerlendirme bağlantısı" alanı (google_maps_url) aslında
   randevu-sonrası WhatsApp mesajında kullanılan bir bağlantı — hiçbir temada
   sitenin kendisinde görünmüyordu. WhatsApp balonuyla aynı düzeyde, her
   temada (ve hizmet detay sayfalarında) tutarlı şekilde görünsün diye tema
