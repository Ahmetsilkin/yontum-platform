...
const MIN_TIME=new Date(1970,0,1,7,0),MAX_TIME=new Date(1970,0,1,22,0);
...
export default function DaySchedule({appointments,staff,services,businessId}:{appointments:Appointment[];staff:any[];services:any[];businessId:string}){
  ...
  const resources=useMemo(()=>visibleStaffList(staff).map(s=>({resourceId:s.id,resourceTitle:s.name})),[staff]);
  const shown=useMemo(()=>localAppts.filter(a=>a.status!=='cancelled'),[localAppts]);
  const events=useMemo<CalEvent[]>(()=>shown.map(a=>({id:a.id,title:`${a.customer_first_name} ${a.customer_last_name}`,start:new Date(a.start_at),end:new Date(a.end_at),resourceId:a.staff_id||'__unassigned',appointment:a})),[shown]);
  ...
  const todaysAppointments=useMemo(()=>shown.filter(a=>a.start_at.slice(0,10)===today),[shown,today]);
  ...
  function applyOptimistic(id:string,startAt:string,endAt:string,staffId:string|null){
    setLocalAppts(prev=>prev.map(a=>a.id===id?{...a,start_at:startAt,end_at:endAt,staff_id:staffId??a.staff_id}:a));
  }
    ...
    <section className="daySchedule">
      <header>
        ...
        <div className="calBadges">
          <div className="calBadge"><small>BUGÜNKÜ CİRO</small><b>{expectedRevenue.toLocaleString('tr-TR')} ₺</b></div>
          <div className="calBadge"><small>BUGÜNKÜ RANDEVU</small><b>{todaysAppointments.length}</b></div>
        </div>
      </header>

      <div className="calTabsRow">
      ...
      <div className="calNav">
        <button onClick={()=>shiftDate(tabMode==='week'?-7:-1)} aria-label="Önceki"><ChevronLeft size={16}/></button>
        <span>{dateLabel}</span>
        <button onClick={()=>shiftDate(tabMode==='week'?7:1)} aria-label="Sonraki"><ChevronRight size={16}/></button>
        <button className="calToday" onClick={()=>setDate(new Date())}>Bugün</button>
      </div>
      ...
      {!shown.length&&<p className="scheduleEmpty">Henüz randevu bulunmuyor.</p>}
