...
export default function DaySchedule({appointments,staff}:{appointments:Appointment[];staff:any[]}){
  ...
  const list=useMemo(()=>appointments.filter(a=>a.start_at.slice(0,10)===date&&a.status!=='cancelled'&&(staffId==='all'||a.staff_id===staffId)).sort((a,b)=>a.start_at.localeCompare(b.start_at)),[appointments,date,staffId]);
    ...
    <section className="daySchedule">
      <header>
        ...
        <select value={staffId} onChange={e=>setStaffId(e.target.value)}>
          ...
          {staff.filter(s=>!s.is_default).map(s=><option key={s.id} value={s.id}>{s.name}</option>)}
        </select>
      </header>
