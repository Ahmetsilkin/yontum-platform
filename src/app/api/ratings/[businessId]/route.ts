...
export async function GET(_:NextRequest,{params}:{params:Promise<{businessId:string}>}){
  ...
  const[{data:summary},{data:reviews}]=await Promise.all([
    ...
    db.from('appointment_ratings').select('stars,comment,customer_name,service_label,avatar_url,created_at').eq('business_id',businessId).not('comment','is',null).order('created_at',{ascending:false}).limit(6),
  ]);
