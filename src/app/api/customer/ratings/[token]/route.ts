...
export async function GET(_:NextRequest,{params}:{params:Promise<{token:string}>}){
  ...
  const{data:ap}=await db.from('appointments')
    .select('id,customer_first_name,start_at,status,service_id,staff_id,business_id,services(name),staff_profiles(name),businesses(name,logo_url,primary_color,background_color,text_color,background_scheme,font_family,selected_theme_id)')
