...
import{buildOverpassQuery,classifyOsm,osmHasWebsite,osmAddress,cleanPhone}from'@/lib/demo-leads';
...
export async function POST(req:NextRequest){
  ...
  try{
    ...
    for(const e of els){
      ...
      rows.push({source:'osm',source_ref:`${e.type}/${e.id}`,name:t.name.slice(0,80),business_type:type,phone:cleanPhone(t.phone||t['contact:phone'])||null,address:osmAddress(t)||null,lat:pt?.lat??null,lng:pt?.lon??null,opening_hours:t.opening_hours||null,has_website:osmHasWebsite(t),area:x.area||null});
    }
